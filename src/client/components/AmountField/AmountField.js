...
const AmountField = ({ handleMinus, handlePlus, handleDelete, value, className, small = false }) => {
    const isDelete = handleDelete && value === 1;
    return (
        <div className={`${styles.amountField} ${small ? styles.smallField : ''} ${className} `}>
            {isDelete ?
                <button onClick={handleDelete} className={styles.btn} >
                    <ion-icon name="trash-outline"></ion-icon>
                </button>
                :
                <button onClick={handleMinus} className={styles.btn} disabled={value <= 1}>
