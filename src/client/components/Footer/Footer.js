...
const Footer = () => {
  ...
  navigation.collections = collections.map((collection) => ({
    label: capitalize(collection.name),
      ...
      <WidthWrapper className={styles.navGrid}>
        {Object.entries(navigation).map(([key, nav]) => (
          <div key={key} className={styles.nav}>
            ...
            {nav.map(({ label, link }) => (
              <LinkSecondary to={link} key={link}>
                {label}
              </LinkSecondary>
            ))}
          </div>
      ...
      <div className={styles.copyrightWrapper}>
        <WidthWrapper>
          <p className={styles.copyright}>
            © 2026 Blush & Blossom. All rights reserved. Designed and developed
            by{' '}
            <a
            ...
            as a portfolio project for demonstration purposes only. The shop and
            products are fictional. All code and content may not be copied,
