...
router.get('/products', async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(500).json({ message: 'Failed to load products. Please try again later.' });
    }
});
...
router.post('/cart', async (req, res) => {
    try {
        ...
        if (!token) {
            return res.status(401).json({ message: 'Please log in to update your cart.' });
        }
...
router.post("/orders/:orderID/capture", async (req, res) => {
    ...
    try {
        ...
        console.error(err);
        res.status(500).json({ message: 'Payment capture failed. Please contact support.' });
    }
});
...
router.get('/orders/:id', async (req, res) => {
    try {
        ...
        if (!token) {
            return res.status(401).json({ message: 'Please log in to view the order.' });
        }
